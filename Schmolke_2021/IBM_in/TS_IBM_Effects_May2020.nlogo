__includes ["GUTS_sublethal_May2020.nls"]

globals [
; lists of waterbody conditions read from input file
; waterbody input files are expected to have 365 entries
  YearlyWaterConditions ; list of water condition files ; ***added 28 May 2020: varying input files for each simulated year are possible
  ...
  temperature ; current temparture of waterbody (pond) in Celsius: from input file
  pondDepth   ; depth of waterbody in m; from input file
  pondVolume  ; water volume; calculated from pondArea and pondDepth

; global variables for effects module (***Effects module, August 2019***)
  YearlyExposureProfiles ; list of file names with yearly exposure profiles ; added 27 May 2020
  ExposureList ; list of daily pesticide water exposure concentrations (ppb or ug/L)
  ; variable for daily values
  pesticideConcentration ; current water pesticide concentration (ppb or ug/L)
  guts_deathcount ; number of deaths in the current time step due to direct mortality (implemented with GUTS)
  list_guts_deathcount ; number of deaths per day due to direct mortality (implemented with GUTS)
  ThreshList ; list of GUTS-IT thresholds
; ***

  YearlyPreyBiomassList  ; list of yearly prey biomass input files ; ***added 28 May 2020: varying input files for each simulated year are possible
...
; prey parameters
  ...
  list_w_ostracods    ; column name in CASM files: Ostracods  ; *** new Sep 2019; CASM-OC
  list_w_corixids     ; column name in CASM files: Corixids ; *** new Sep 2019; CASM-OC
  ...
  list_w_coleopterans ; column name in CASM files: Coleopterans ; *** new Sep 2019; CASM-OC
  ...
  preyatthestartoftheday ; biomass in g C per m2 of prey guild at the start of the day (before consumption by TS); *** Sep 2019: includes 13 guilds that are reduced to 10 TS prey groups ***
  preyattheendoftheday   ; biomass in g C per m2 of prey guild at the end of the day (after consumption by TS); *** Sep 2019: includes 13 guilds that are expanded from 10 TS prey groups ***
...
;;; effects module
  t_intConc_GUTS  ; internal concentration (GUTS variable) ***Effects module, August 2019***
  guts_it_thresh  ; inidividual survival threshold (GUTS-IT) ***Effects module, August 2019***
  t_intConc_sublethal ; internal concentration (TKTD variable for sublethal effects) ***Effects module, August 2019***
  sublethal_stress ; sublethal stress experienced by individual due to persticide exposure (affects Cmax)
]
...
to setup
  ...
  if (MultiYearInputs = TRUE) [
    readWaterConditionInputList ; reads in list of input files; ***added 28 May 2020
    if (DailyIO = FALSE) [ readPreyBiomassInputList ]  ; reads in list of input files, only applicable if dialy biomass input/output with CASM not switched on; ***added 28 May 2020
  ]
...
; Effects module (***Effects module, Aug 2019***): read in daily water concentrations if exposure is not set to a constant value
  if (EffectsModule) [
  ; added 27 May 2020: exposure can be set to constant or to a list of yearly files
  ; Constant exposure is set in the 'go' procedure daily
    if (ConstantExposure = FALSE) [
      if (MultiYearInputs = TRUE) [ readExposureProfileList ]
      readExposureConcentrations
    ]
;    if (Effects = "GUTS-IT" or Effects = "GUTS-IT + Sublethal") [ readGUTS_IT_thresholds ]
    if (Effects = "GUTS-IT" or Effects = "GUTS-IT + Sublethal") [ set ThreshList GUTS_IT_tresholds ]
    set guts_deathcount 0
    set list_guts_deathcount []
  ]
; ***
  ...
  ifelse (DailyIO) [
    ...
    if (MultiYearInputs = TRUE) [ readPreyBiomassInputList ]
  ...
  create-turtles num_subadults [
    ...
    set spawn_date -1
    set size 3
    ; ***Effects module, August 2019***
    set t_intConc_GUTS 0
    if (EffectsModule = TRUE) [
      if (Effects = "GUTS-IT" or Effects = "GUTS-IT + Sublethal") [
        with-local-randomness [
          let p round ((random-float 1) * 1000)
          ifelse (p = 1000)
          [ set guts_it_thresh item 999 ThreshList ]
          [ set guts_it_thresh item p ThreshList ]
;      type p type ", conc: " print guts_it_thresh
        ]
      ]
    ]
    set t_intConc_sublethal 0
    ; ***
  ]
...
;      type p type ", conc: " print guts_it_thresh
  ...
  if WriteGrowthTestOutput [
    ...
    file-write "t_intConc_GUTS"  ; internal concentration (GUTS variable) ***Effects module, August 2019***
    file-write "guts_it_thresh"  ; inidividual survival threshold (GUTS-IT) ***Effects module, August 2019***
    file-write "t_intConc_sublethal" ; internal concentration (TKTD variable for sublethal effects) ***Effects module, August 2019***
    file-write "sublethal_stress" ; sublethal stress experienced by individual due to persticide exposure (affects Cmax)
...
; Effects module (***Effects module, Aug 2019***): read in daily water concentrations
  if (EffectsModule) [
  ; added 27 May 2020: exposure can be set to constant or to a list of yearly files
  ; Constant exposure is set in the 'go' procedure daily
    if (ConstantExposure = FALSE) [
...
to go
  ...
  if ticks mod 365 = 0 [
    ...
    if (MultiYearInputs = TRUE) [
      readWaterConditions ; read in year-specific water conditions
      if (DailyIO = FALSE) [ readDailyPreyBiomass_year ] ; read in year-specific daily prey biomasses (if dialy input/output from CASM not switched on)
    ]
...
; Effects module (***Effects module, Aug 2019***): read in daily water concentrations
  if (EffectsModule) [
  ; added 27 May 2020: exposure can be set to constant or to a list of yearly files
    ifelse (ConstantExposure = TRUE)
    [ set pesticideConcentration ConstantExposureLevel ] ; constant exposure defined on the interface
    [
      if ticks mod 365 = 0 [ readExposureConcentrations ]
      set pesticideConcentration item (ticks mod 365) ExposureList ; set exposure from list with daily values
    ]
    set guts_deathcount 0
  ]
; ***
...
;    type "Fish eaten by largemouth bass, weight: " type TS_eaten_cumulative type ", number: " print TS_eaten_num ; for testing
  ...
  ask turtles [
    ...
    ; Effects application (***Effects module, Aug 2019***)
  if (EffectsModule) [
    if (Effects = "GUTS-SD") or (Effects = "GUTS-SD + Sublethal") [ with-local-randomness[ ask turtles with [stage != "egg"] [GUTS_SD] ] ]
    if (Effects = "GUTS-IT") or (Effects = "GUTS-IT + Sublethal") [ with-local-randomness[ ask turtles with [stage != "egg"] [GUTS_IT] ] ]
;    type "deathcount: " print guts_deathcount
    set list_guts_deathcount lput guts_deathcount list_guts_deathcount
    if (Effects = "Sublethal") or (Effects = "GUTS-SD + Sublethal") or (Effects = "GUTS-IT + Sublethal") [
      with-local-randomness[ ask turtles with [stage != "egg"] [set sublethal_stress sublethal_TKTD] ]
    ]
  ]
  ; ***
  ...
  ifelse CmaxConsumption [
    ...
    ; Effects application (***Effects module, Aug 2019***)
      if (EffectsModule) [
        if (Effects = "Sublethal") or (Effects = "GUTS-SD + Sublethal") or Effects = ("GUTS-IT + Sublethal") [
          set day_consumption day_consumption * (1 - sublethal_stress)
        ]
      ]
  ; ***
      grow
  ...
  ; consumption according to functional response from CASM
    ask turtles with [stage = "juvenile" or stage = "subadult" or stage = "adult"] [
      functional_response_CASM ; ***Effects module, Aug 2019***: sublethal effects applied to Cmax within this procedure
      grow
    ]
  ]
  ...
  ; biomass conversion for output moved to separate procedure (***Effects module, Aug 2019***)
  convertBiomassesForOutput
...
;    type "Cmax_t: " type Cmax_t type ", fC_T: " print fC_T; for testing
  ...
  if (EffectsModule) [
    if (Effects = "Sublethal") or (Effects = "GUTS-SD + Sublethal") or Effects = ("GUTS-IT + Sublethal") [
      set Cmax_t Cmax_t * (1 - sublethal_stress)
    ]
  ]
  ; ***
...
;  set day_consumption (C_copepods * ED_copepods) + (C_cladocerans * ED_cladocerans) + (C_rotifers * ED_rotifers)
 ;                   + (C_microzoo * ED_microzoo) + (C_detritus * ED_detritus)
    ; Consumption per prey type dependent on prey biomass *** Sep 2019: AND THEIR ED !!! ***
   ...
   ; consumption rates in g prey guild per fish and day *** Sep 2019: divided by sum_rel_C ***
   let C_copepods W * ww_Cmax_juv * (juv_pref_copepods * juv_ass_copepods * juv_h_copepods * w_copepods * cfv) / sum_rel_C ; (1 + sum_rel_C)
   if C_copepods > W * ww_Cmax_juv [
      print "CASM functional response: copepod consumption rate too high!"
     user-message "CASM functional response: copepod consumption rate too high!"
     stop
   ]
   let C_cladocerans W * ww_Cmax_juv * (juv_pref_cladocerans * juv_ass_cladocerans * juv_h_cladocerans * w_cladocerans * cfv) / sum_rel_C ; (1 + sum_rel_C)
   if C_cladocerans > W * ww_Cmax_juv [
      print "CASM functional response: cladoceran consumption rate too high!"
     user-message "CASM functional response: cladoceran consumption rate too high!"
     stop
   ]
   let C_rotifers W * ww_Cmax_juv * (juv_pref_rotifers * juv_ass_rotifers * juv_h_rotifers * w_rotifers * cfv) / sum_rel_C ; (1 + sum_rel_C)
   if C_rotifers > W * ww_Cmax_juv [
      print "CASM functional response: rotifers consumption rate too high!"
     user-message "CASM functional response: rotifers consumption rate too high!"
     stop
   ]
   let C_microzoo W * ww_Cmax_juv * (juv_pref_microzoo * juv_ass_microzoo * juv_h_microzoo * w_microzoo * cfv) / sum_rel_C ; (1 + sum_rel_C)
   if C_microzoo > W * ww_Cmax_juv [
      print "CASM functional response: microzooplankton consumption rate too high!"
     user-message "CASM functional response: microzooplankton consumption rate too high!"
     stop
   ]
   let C_detritus W * ww_Cmax_juv * (juv_pref_detritus * juv_ass_detritus * juv_h_detritus * w_detritus * cfa) / sum_rel_C ; (1 + sum_rel_C) ; (SedimentPOC)
   if C_detritus > W * ww_Cmax_juv [
      print "CASM functional response: detritus consumption rate too high!"
   ...
   if (C_cladocerans < 0 or C_copepods < 0 or C_rotifers < 0 or C_microzoo < 0 or C_detritus < 0) [ ; (SedimentPOC)
      print "WARNING: negative consumption by juvenile"
     user-message "WARNING: negative consumption by juvenile"
   ]
    ; *** Sep 2019
    if ((C_cladocerans + C_copepods + C_rotifers + C_microzoo + C_detritus) > (W * ww_Cmax_juv * 1.001)) [
      type "Cmax: " type ww_Cmax_juv type ", Cmax * weight: " type W * ww_Cmax_juv type ", sum consumed prey: " print (C_cladocerans + C_copepods + C_rotifers + C_microzoo + C_detritus)
      print "WARNING: effective consumption exceeeds Cmax in juvenile"
      user-message "WARNING: effective consumption exceeeds Cmax in juvenile"
    ]
    ; ***
  ...
  ; daily consumption of the juvenile (in J); note that Cmax is in g (wet weight):
  set day_consumption (C_copepods * ED_copepods) + (C_cladocerans * ED_cladocerans) + (C_rotifers * ED_rotifers)
                    + (C_microzoo * ED_microzoo) + (C_detritus * ED_detritus)
  ...
  ; consumption rates in g prey/ g fish and day
    let C_copepods W * ww_Cmax_adt * (adt_pref_copepods * adt_ass_copepods * adt_h_copepods * w_copepods * cfv) / sum_rel_C ; (1 + sum_rel_C)
    if C_copepods > W * ww_Cmax_adt [
      print "CASM functional response: copepod consumption rate too high!"
      user-message "CASM functional response: copepod consumption rate too high!"
      stop
    ]
    let C_cladocerans W * ww_Cmax_adt * (adt_pref_cladocerans * adt_ass_cladocerans * adt_h_cladocerans * w_cladocerans * cfv) / sum_rel_C ; (1 + sum_rel_C)
    if C_cladocerans > W * ww_Cmax_adt [
      print "CASM functional response: cladoceran consumption rate too high!"
      user-message "CASM functional response: cladoceran consumption rate too high!"
      stop
    ]
    let C_mayflies W * ww_Cmax_adt * (adt_pref_mayflies * adt_ass_mayflies * adt_h_mayflies * w_mayflies * cfa) / sum_rel_C ; (1 + sum_rel_C) ;  (Ephemeroptera)
    if C_mayflies > W * ww_Cmax_adt [
      print "CASM functional response: mayfly consumption rate too high!"
      user-message "CASM functional response: mayfly consumption rate too high!"
      stop
    ]
    let C_caddisflies W * ww_Cmax_adt * (adt_pref_caddisflies * adt_ass_caddisflies * adt_h_caddisflies * w_caddisflies * cfa) / sum_rel_C ; (1 + sum_rel_C) ; (Trichoptera)
    if C_caddisflies > W * ww_Cmax_adt [
      print "CASM functional response: caddisfly consumption rate too high!"
      user-message "CASM functional response: caddisfly consumption rate too high!"
      stop
    ]
    let C_oligochaetes W * ww_Cmax_adt * (adt_pref_oligochaetes * adt_ass_oligochaetes * adt_h_oligochaetes * w_oligochaetes * cfa) / sum_rel_C ; (1 + sum_rel_C)
    if C_oligochaetes > W * ww_Cmax_adt [
      print "CASM functional response: oligochaete consumption rate too high!"
      user-message "CASM functional response: oligochaete consumption rate too high!"
      stop
    ]
    let C_chironomids W * ww_Cmax_adt * (adt_pref_chironomids * adt_ass_chironomids * adt_h_chironomids * w_chironomids * cfa) / sum_rel_C ; (1 + sum_rel_C)
    if C_chironomids > W * ww_Cmax_adt [
      print "CASM functional response: chironomid consumption rate too high!"
      user-message "CASM functional response: chironomid consumption rate too high!"
    stop
    ]
    let C_detritus W * ww_Cmax_adt * (adt_pref_detritus * adt_ass_detritus * adt_h_detritus * w_detritus * cfa) / sum_rel_C ; (1 + sum_rel_C) ; (SedimentPOC)
    if C_detritus > W * ww_Cmax_adt [
      print "CASM functional response: detritus consumption rate too high!"
      user-message "CASM functional response: detritus consumption rate too high!"
      stop
    ]
    let C_periphyton W * ww_Cmax_adt * (adt_pref_periphyton * adt_ass_periphyton * adt_h_periphyton * w_periphyton * cfv) / sum_rel_C ; (1 + sum_rel_C)
    if C_periphyton > W * ww_Cmax_adt [
      print "CASM functional response: periphyton consumption rate too high!"
  ...
  if (C_copepods < 0 or C_cladocerans < 0 or C_mayflies < 0 or C_caddisflies < 0 or C_oligochaetes < 0 or C_chironomids < 0 or C_detritus < 0 or C_periphyton < 0) [
      print "WARNING: negative consumption by adult"
      user-message "WARNING: negative consumption by adult"
  ]
    ; *** Sep 2019
    if ((C_copepods + C_cladocerans + C_mayflies + C_caddisflies + C_oligochaetes + C_chironomids + C_detritus + C_periphyton) > (W * ww_Cmax_adt * 1.001)) [
      type "Cmax: " type ww_Cmax_juv type ", Cmax * weight: " type W * ww_Cmax_adt type ", sum consumed prey: " print (C_copepods + C_cladocerans + C_mayflies + C_caddisflies + C_oligochaetes + C_chironomids + C_detritus + C_periphyton)
      print "WARNING: effective consumption exceeeds Cmax in adult"
      user-message "WARNING: effective consumption exceeeds Cmax in adult"
    ]
    ; ***
  ; calculate actual consumption (by a fish), and reduce prey guild biomasses accordingly
    ifelse C_copepods > w_copepods [
...
to setDailyPreyBiomass
  ...
  set preyatthestartoftheday [] ; *** needs to change! Sep 2019
  ...
  set preyatthestartoftheday lput (item (ticks mod 365) list_w_ostracods) preyatthestartoftheday ; *** Sep 2019, CASM-OC ***
  set preyatthestartoftheday lput (item (ticks mod 365) list_w_corixids) preyatthestartoftheday ; *** Sep 2019, CASM-OC ***
  ...
  set preyatthestartoftheday lput (item (ticks mod 365) list_w_coleopterans) preyatthestartoftheday ; *** Sep 2019, CASM-OC ***
  ...
  set w_cladocerans (((item (ticks mod 365) list_w_cladocerans) + ((item (ticks mod 365) list_w_ostracods))) * gC_ww_cladocerans * pondArea) ; *** Sep 2019, CASM-OC: for TS consumption, cladocerans and ostracods are combined ***
  ...
  set w_caddisflies (((item (ticks mod 365) list_w_corixids) + (item (ticks mod 365) list_w_caddisflies) + (item (ticks mod 365) list_w_coleopterans)) * gC_ww_caddisflies * pondArea) ; (Trichoptera); *** Sep 2019, CASM-OC: for TS consumption, trichopterans are combined with corixids and coleopterans ***
...
to convertBiomassesForOutput
  ; Moved from 'go' procedure (***Effects module, Aug2019***)
  ...
  with-local-randomness [ ;; local randomness added here because these are only related to outputs: calling sum of turtles uses random number generator
    ; *** Sep 2019, CASM-OC: recalculate cladoceran and ostracod biomasses
    let gC_clado (w_cladocerans / (pondArea * gC_ww_cladocerans))
    let start_gC_co_combined (item 1 preyatthestartoftheday) + (item 4 preyatthestartoftheday)
    let ratio_co_endstart gC_clado / start_gC_co_combined
    let gC_caddis (w_caddisflies / (pondArea * gC_ww_caddisflies))
    let start_gC_ctc_combined (item 5 preyatthestartoftheday) + (item 7 preyatthestartoftheday) + (item 10 preyatthestartoftheday)
    let ratio_ctc_endstart gC_caddis / start_gC_ctc_combined
    ; *** end ***
    set preyattheendoftheday []
    set preyattheendoftheday lput (w_copepods / (pondArea * gC_ww_copepods)) preyattheendoftheday
    set preyattheendoftheday lput ((item 1 preyatthestartoftheday) * ratio_co_endstart) preyattheendoftheday ; *** Sep 2019, CASM-OC ***
    ...
    set preyattheendoftheday lput ((item 4 preyatthestartoftheday) * ratio_co_endstart) preyattheendoftheday ; *** Sep 2019, CASM-OC ***
    set preyattheendoftheday lput ((item 5 preyatthestartoftheday) * ratio_ctc_endstart) preyattheendoftheday ; *** Sep 2019, CASM-OC ***
    set preyattheendoftheday lput (w_mayflies / (pondArea * gC_ww_mayflies)) preyattheendoftheday ;  (Ephemeroptera)
    set preyattheendoftheday lput ((item 7 preyatthestartoftheday) * ratio_ctc_endstart) preyattheendoftheday ; *** Sep 2019, CASM-OC ***
    ...
    set preyattheendoftheday lput ((item 10 preyatthestartoftheday) * ratio_ctc_endstart) preyattheendoftheday ; *** Sep 2019, CASM-OC ***
    ...
    ifelse flag_limit_detritus  [
      ...
      if detritus_cons < 0 [
        print "detritus consumption negative! (from 'convertBiomassesForOutput' procedure)"
        user-message "detritus consumption negative! (from 'convertBiomassesForOutput' procedure)"
      ]
    ...
    set TS_W lput ((sum [W] of turtles with [stage = "larva" and sex = "female"]) / (pondArea * gC_ww_fish)) TS_W ; note that using fish conversion factor might not be accurate to apply to larva weight
    set TS_W lput ((sum [W] of turtles with [stage = "larva" and sex = "male"]) / (pondArea * gC_ww_fish)) TS_W ; note that using fish conversion factor might not be accurate to apply to larva weight
    set TS_W lput ((sum [W] of turtles with [stage = "juvenile" and sex = "female"]) / (pondArea * gC_ww_fish)) TS_W ; note that using fish conversion factor might not be accurate to apply to juvenile weight
    set TS_W lput ((sum [W] of turtles with [stage = "juvenile" and sex = "male"]) / (pondArea * gC_ww_fish)) TS_W ; note that using fish conversion factor might not be accurate to apply to juvenile weight
...
to readWaterConditions
  ; added 27 May 2020: list of files as input for water conditions
  ...
  let WaterConditionInput_year WaterConditionInput
  if (MultiYearInputs = TRUE) [
    let year ticks / 365
;  type "current year: " print year
    set WaterConditionInput_year item year YearlyWaterConditions
;  type "reading water condition input: " print WaterConditionInput_year
  ]

  set TemperatureList []
  set WaterDepthList []

  ifelse file-exists? WaterConditionInput_year [
    file-open WaterConditionInput_year
    let entry ""
    let cnt 0
...
;      print entry ; for testing
    ...
    print "Waterbody condition input file does not exist in current directory!"
    user-message "Waterbody condition input file does not exist in current directory!"
    stop
  ]
  if (length TemperatureList) > 365 [
    print "Temperature input larger than 365 entries: only first 365 entries used."
  ...
  if length TemperatureList < 365 [
    print "Temperature input less than 365 entries: input file cannot be used."
    user-message "Temperature input less than 365 entries: input file cannot be used."
    stop
  ]
  if (length WaterDepthList) > 365 [
    print "Water depth input larger than 365 entries: only first 365 entries used."
  ...
  if length WaterDepthList < 365 [
    print "Water depth input less than 365 entries: input file cannot be used."
    user-message "Water depth input less than 365 entries: input file cannot be used."
    stop
  ]
end

to readWaterConditionInputList
  ; Effects module (***Exposure module, August 2019***)
  ; added 27 May 2020: allowing for different yearly exposure profiles
  set YearlyWaterConditions []

  ifelse (file-exists? WaterConditionInput) [
    file-open WaterConditionInput
    let num_WaterCondition_files file-read ; Read in the number of WaterCondition files
    set YearsToRun num_WaterCondition_files ; reset years to run to the number of yearly WaterCondition input files
    repeat num_WaterCondition_files [ ; read in all the exposure profile file names into a list
      set YearlyWaterConditions lput file-read-line YearlyWaterConditions
    ]
    file-close
  ] [
    print "WARNING: file WaterConditionInput not found in current directory."
    user-message ("WARNING: file WaterConditionInput not found in current directory.")
    stop
  ]

end

to readExposureConcentrations
  ; Effects module (***Exposure module, August 2019***)

  ;  type "current year: " print year
  let ExposureProfile_year ExposureProfile

  if (MultiYearInputs = TRUE) [
    let year ticks / 365
;  type "current year: " print year
    set ExposureProfile_year item year YearlyExposureProfiles
;  type "reading exposure profile: " print ExposureProfile
  ]

  set ExposureList []

  ifelse file-exists? ExposureProfile_year [
    file-open ExposureProfile_year
    let entry ""
    let cnt 0
    while [ cnt < 3 ] [ ; top 3 lines of file are file info
      set entry file-read-line
 ;     print entry ; for testing
      set cnt cnt + 1
    ]
    ;; the following section checks whether the pesticide name is stated as column header
    ;; NOTE: the second column is subsequently read in by column number, not by name in the header (see below)!!!
    set entry file-read-line
 ;   type "ExposureProfile column headers: " print entry ; for testing
    if (not member? PesticideName entry) [
      print "Pesticide name does not correspond to name listed in exposure input file or file header has changed."
      user-message "Pesticide name does not correspond to name listed in exposure input file or file header has changed."
    ]

    set cnt 0
    while [ cnt < 365 ] [ ;
      set entry file-read ; exposure is listed in the second column in the file
      set ExposureList lput file-read ExposureList
      set entry file-read-line
      set cnt cnt + 1
    ]
    ...
    print "Exposure profile input file does not exist in current directory!"
    user-message "Exposure profile input file does not exist in current directory!"
    stop
  ]

  if (length ExposureList) > 365 [
    print "Exposure profile input larger than 365 entries: only first 365 entries used."
    user-message "Exposure profile input larger than 365 entries: only first 365 entries used."
    set ExposureList (sublist ExposureList 0 365)
  ]
  if length ExposureList < 365 [
    print "Exposure profile input less than 365 entries: input file cannot be used."
    user-message "Exposure profile input less than 365 entries: input file cannot be used."
    stop
  ]
end

to readExposureProfileList
  ; Effects module (***Exposure module, August 2019***)
  ; added 27 May 2020: allowing for different yearly exposure profiles
  set YearlyExposureProfiles []

  ifelse (file-exists? ExposureProfile) [
    file-open ExposureProfile
    let num_Exposure_files file-read ; Read in the number of exposure profile files
    if (YearsToRun != num_Exposure_files) [
        print "WARNING: number of years to run differs between water condition input and exposure input."
        user-message ("WARNING: number of years to run differs between water condition input and exposure input.")
        stop
    ]
    repeat num_Exposure_files [ ; read in all the exposure profile file names into a list
      set YearlyExposureProfiles lput file-read-line YearlyExposureProfiles
    ]
    file-close
  ] [
    print "WARNING: file ExposureProfileList not found in current directory."
    user-message ("WARNING: file ExposureProfileList not found in current directory.")
    stop
  ]
end

; removed 27 May 2020: procedure is obsolete because GUTS-IT thresholds are calculated by 'GUTS_IT_thresholds'
;to readGUTS_IT_thresholds
;  ; reading in internal concentration threshold look-up table
;  let ProbList []
;  set ThreshList []

;  ifelse file-exists? "Solatenol_GUTS_IT_thresholds.txt" [
;    file-open "Solatenol_GUTS_IT_thresholds.txt"
;    let entry file-read-line ; column headers
;      print entry ; for testing
;   let cnt 0
;    while [ cnt < 1000 ] [
;      set ProbList lput file-read ProbList
;      set ThreshList lput file-read ThreshList
;      set cnt cnt + 1
;    ]
;    file-close
;  ] [
;      user-message ("WARNING: file Solatenol_GUTS_IT_thresholds.txt not found in current directory.")
;  ]
;end
...
to readDailyPreyBiomass_year
  ...
  let DailyPreyBiomass_year DailyPreyBiomass
  if (MultiYearInputs = TRUE) [
    let year ticks / 365
;  type "current year: " print year
    set DailyPreyBiomass_year item year YearlyPreyBiomassList
;  type "reading exposure profile: " print DailyPreyBiomass_year
  ]
  ...
  set list_w_rotifers []
  set list_w_microzoo []
  set list_w_ostracods [] ; *** added Sep 2019, CASM-OC ***
  set list_w_corixids []  ;  (Corixida); *** added Sep 2019, CASM-OC ***
  set list_w_mayflies []  ;  (Ephemeroptera)
  set list_w_caddisflies [] ; (Trichoptera)
  set list_w_oligochaetes []
  set list_w_chironomids []
  set list_w_coleopterans [] ; *** added Sep 2019, CASM-OC ***
  ...
  ifelse file-exists? DailyPreyBiomass_year [
    file-open DailyPreyBiomass_year
    let entry ""
    let cnt 0
...
;    type "copepods - flag_prey: " print flag_prey ; for testing
    if (not member? "Cladocerans" entry) [ set flag_prey true ]
...
; *** Sep 2019; CASM-OC ***
    if (not member? "Ostracods" entry) [ set flag_prey true ]
;    type "Ostracods - flag_prey: " print flag_prey ; for testing
; *** end ***
; *** Sep 2019; CASM-OC ***
    if (not member? "Corixids" entry) [ set flag_prey true ]
;    type "Corixids - flag_prey: " print flag_prey ; for testing
; *** end ***
...
; *** Sep 2019; CASM-OC ***
    if (not member? "Coleopterans" entry) [ set flag_prey true ]
;    type "Coleopterans - flag_prey: " print flag_prey ; for testing
; *** end ***
...
;    type "TotalPeriphyt - flag_prey: " print flag_prey ; for testing
    ...
    if (flag_prey) [
      print "WARNING: prey biomass input from file incomplete!"
      user-message "WARNING: prey biomass input from file incomplete!"
      stop
    ]
;    type "flag_TS_eaten: " print flag_TS_eaten ; for testing
; *** Sep 2019; CASM-OC: all columns in CASM output file have changed! ***
    set cnt 0
    while [not file-at-end?] [
      let item_cnt 0
      while [ item_cnt < 13 ] [
        set entry file-read
        set item_cnt item_cnt + 1
 ;       type "entry read from file: " type entry type "; # of entry read: " print item_cnt ; for testing
      ]
      set list_w_copepods lput (file-read) list_w_copepods ; column 14 in input file
      set item_cnt item_cnt + 1
      set list_w_cladocerans lput (file-read) list_w_cladocerans ; column 15 in input file
      set item_cnt item_cnt + 1
      set list_w_rotifers lput (file-read) list_w_rotifers ; column 16 in input file
      set item_cnt item_cnt + 1
      set list_w_microzoo lput (file-read) list_w_microzoo ; column 17 in input file
      set item_cnt item_cnt + 1
      set list_w_ostracods lput (file-read) list_w_ostracods ; column 18 in input file ; *** CASM-OC includes ostracods which are combined with cladoceran biomass
      set item_cnt item_cnt + 1
      while [ item_cnt < 21 ] [
        set entry file-read
        set item_cnt item_cnt + 1
      ]
      set list_w_TS_juv lput (file-read * gC_ww_fish * pondArea) list_w_TS_juv ; column 22 in input file
      set item_cnt item_cnt + 1
      set list_w_TS_adt lput (file-read * gC_ww_fish * pondArea) list_w_TS_adt ; column 23 in input file
      set item_cnt item_cnt + 1
      while [ item_cnt < 27 ] [
        set entry file-read
        set item_cnt item_cnt + 1
      ]
      set list_w_corixids lput (file-read) list_w_corixids ; column 28 in input file ; *** CASM-OC includes corixids and colepoterans which are combined with trichopteran biomass
      set item_cnt item_cnt + 1
      set entry file-read
      set item_cnt item_cnt + 1
      set list_w_mayflies lput (file-read) list_w_mayflies ; column 30 in input file ;  (Ephemeroptera)
      set item_cnt item_cnt + 1
      set list_w_caddisflies lput (file-read) list_w_caddisflies ; column 31 in input file
      set item_cnt item_cnt + 1
      set list_w_oligochaetes lput (file-read) list_w_oligochaetes ; column 32 in input file
      set item_cnt item_cnt + 1
      set list_w_chironomids lput (file-read) list_w_chironomids ; column 33 in input file
      set item_cnt item_cnt + 1
      set list_w_coleopterans lput (file-read) list_w_coleopterans ; column 34 in input file ; *** CASM-OC includes corixids and colepoterans which are combined with trichopteran biomass
      set item_cnt item_cnt + 1
      while [ item_cnt < 47 ] [
        set entry file-read
        set item_cnt item_cnt + 1
      ...
      set list_w_detritus lput detritus_tmp list_w_detritus ; column 48 in input file ("Sediment POC")
      set item_cnt item_cnt + 1
      set entry file-read
      set item_cnt item_cnt + 1
      set list_w_periphyton lput (file-read) list_w_periphyton ; column 50 in input file ("Total periphyton")
      set item_cnt item_cnt + 1
      ifelse flag_TS_eaten [
        ...
        set list_TS_eaten lput (file-read) list_TS_eaten ; column 78 in input file (TSgww - eaten)
        set entry file-read-line
      ]
      set cnt cnt + 1
 ;     type "TS_eaten: " print (last list_TS_eaten)
 ;     print cnt ; for testing
    ]
  ] [
    print "Daily prey biomass input file does not exist in current directory!"
  ...
  if ( (length list_w_copepods) != 365 or (length list_w_cladocerans) != 365
    ...
    print "WARNING: daily prey biomass input does not have 365 entries!"
    user-message "WARNING: daily prey biomass input does not have 365 entries!"
    stop
  ]
end

to readPreyBiomassInputList
  ; Effects module (***Exposure module, August 2019***)
  ; added 27 May 2020: allowing for different yearly prey biomass inputs
  set YearlyPreyBiomassList []

  ifelse (file-exists? DailyPreyBiomass) [
    file-open DailyPreyBiomass
    let num_PreyBiomass_files file-read ; Read in the number of yearly prey biomass input files
    if (MultiYearInputs = TRUE) [
      if (YearsToRun != num_PreyBiomass_files) [
        print "WARNING: number of years to run differs between water condition input and prey biomass input."
        user-message ("WARNING: number of years to run differs between water condition input and prey biomass input.")
        stop
      ]
    ]
    repeat num_PreyBiomass_files [ ; read in all the yearly prey biomass input file names into a list
      set YearlyPreyBiomassList lput file-read-line YearlyPreyBiomassList
    ]
    file-close
  ] [
    print "WARNING: file DailyPreyBiomass not found in current directory."
    user-message ("WARNING: file DailyPreyBiomass not found in current directory.")
    stop
  ]
...
;    type "flag_TS_eaten: " print flag_TS_eaten ; for testing
    ...
    let copepods_temp file-read  ; column 12 in input file
    set preyatthestartoftheday lput copepods_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let clado_temp file-read  ; column 13 in input file
    set preyatthestartoftheday lput clado_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let rotifers_temp file-read ; column 14 in input file
    set preyatthestartoftheday lput rotifers_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let microzoo_temp file-read  ; column 15 in input file
    set preyatthestartoftheday lput microzoo_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let ostracod_temp file-read ; column 16 in input file
    set preyatthestartoftheday lput ostracod_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    set w_copepods (copepods_temp * gC_ww_copepods * pondArea)
    set w_cladocerans ((clado_temp + ostracod_temp) * gC_ww_cladocerans * pondArea) ; *** Sep 2019, CASM-OC: cladocerans and ostracods combined as TS prey item
    set w_rotifers (rotifers_temp * gC_ww_rotifers * pondArea)
    set w_microzoo (microzoo_temp * gC_ww_microzoo * pondArea)
    while [ item_cnt < 19 ] [
      set entry file-read
      set item_cnt item_cnt + 1
    ]
    set ini_w_TS_juv (file-read * gC_ww_fish * pondArea) ; column 20 in input file
    set item_cnt item_cnt + 1
    set ini_w_TS_adt (file-read * gC_ww_fish * pondArea) ; column 21 in input file
    set item_cnt item_cnt + 1
    while [ item_cnt < 25 ] [
      set entry file-read
      set item_cnt item_cnt + 1
    ]
; *** Sep 2019, CASM-OC; NOTE: preyatthestartoftheday does not explicitly list ostracods, corixids or coleopterans! ***
; *** instead, ostracod biomass is combined with cladocerans; corixids and coleopterans are combined with caddisflies (Tricoptera) ***
    let corixids_temp file-read  ; column 26 in input file ;  (Corixida)
    set preyatthestartoftheday lput corixids_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    set entry file-read ; column not used
    set item_cnt item_cnt + 1
    let mayflies_temp file-read ; column 28 in input file ;  (Ephemeroptera)
    set preyatthestartoftheday lput mayflies_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let caddisflies_temp file-read  ; column 29 in input file ; (Trichoptera)
    set preyatthestartoftheday lput caddisflies_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let oligos_temp file-read ; column 30 in input file ; (Oligochaeta)
    set preyatthestartoftheday lput oligos_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let chirono_temp file-read ; column 31 in input file ; (Chironomida)
    set preyatthestartoftheday lput chirono_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    let coleopt_temp file-read  ; column 32 in input file ;  (Coleoptera)
    set preyatthestartoftheday lput coleopt_temp preyatthestartoftheday
    set item_cnt item_cnt + 1
    set w_mayflies (mayflies_temp * gC_ww_mayflies * pondArea)
    set w_caddisflies ((corixids_temp + caddisflies_temp + coleopt_temp) * gC_ww_caddisflies * pondArea) ; caddisflies combined diet item
    set w_oligochaetes (oligos_temp * gC_ww_oligochaetes * pondArea)
    set w_chironomids (chirono_temp * gC_ww_chironomids * pondArea)
    while [ item_cnt < 45 ] [
      set entry file-read
      set item_cnt item_cnt + 1
    ]
...
;    type "Sediment POC entry: " print entry
    ...
    set w_detritus (entry * gC_ww_detritus * pondArea) ; column 46 in input file ("Sediment POC")
...
;    type "w_detritus after threshold application: " print w_detritus ; for testing
    ...
    set entry file-read
    set preyatthestartoftheday lput entry preyatthestartoftheday
    set w_periphyton (entry * gC_ww_periphyton * pondArea) ; column 48 in input file ("Total periphyton")
    set item_cnt item_cnt + 1
    ifelse flag_TS_eaten [
      set w_TS_eaten 0 ; if TS eaten is not listed in the prey biomass input file, it is assumed that no predation occurs
      set entry file-read-line
    ] [
      while [ item_cnt < 75 ] [ ; *** verify column number! ***;  it is unclear to me what's happening here, but I get the right column with this (even though TSgww-eaten is in copumn 73)
        set entry file-read
        set item_cnt item_cnt + 1
      ]
      set w_TS_eaten (file-read * pondVolume) ; column 76 in input file (TSgww - eaten); note: biomass in g wet weight/m3 (not gC/m3)
    ]
  ] [
    print "Daily prey biomass input file does not exist in current directory!"
    user-message "Daily prey biomass input file does not exist in current directory!"
...
665
DailyPreyBiomass
IBM_TS_transfer_list_test_28May2020.txt
1
0
String
...
192
BiomassOutput
biomass_output_combEffects_29May2020b_9.txt
1
0
String
...
593
WaterConditionInput
env_input_list_test_28May2020.txt
1
0
String
...
1176
FishGrowthTest
Sublethal_9_4_Cmax25C_6Mar2020_m.txt
1
0
String
...
838
ExposureProfile
Solatenol_exposure_list_set1.txt
1
0
String
...
Effects
Effects
"GUTS-SD" "GUTS-IT" "Sublethal" "GUTS-SD + Sublethal" "GUTS-IT + Sublethal"
2
...
"" ""
PENS
"Pesticide concentration (ppb)" 1.0 0 -16777216 true "" "plot pesticideConcentration"
"GUTS deaths" 1.0 0 -2674135 true "" "plot guts_deathcount"
...
347
658
If MultiYearInputs = off:\nWaterConditionInput and DailyPreyBiomass expect name of input files\nIf MultiYearInputs = on:\nWaterConditionInput and DailyPreyBiomass expects name of file with list of input files
10
0.0
1
