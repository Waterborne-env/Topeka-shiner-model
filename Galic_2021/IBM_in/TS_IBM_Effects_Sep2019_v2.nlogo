__includes ["GUTS_sublethal_Sep2019_v1.nls"]
...
;    [ user-message "WARNING: sublethal effects not yet implemented!" ]
...
; Effects module (***Effects module, Aug 2019***): read in daily water concentrations
  if (EffectsModule) [
    readExposureConcentrations
    if (Effects = "GUTS-IT") [ readGUTS_IT_thresholds ]
    set list_guts_deathcount []
  ]
...
; Effects module (***Effects module, Aug 2019***): read in daily water concentrations
  if (EffectsModule) [
    set pesticideConcentration item (ticks mod 365) ExposureList
...
;    type "Fish eaten by largemouth bass, weight: " type TS_eaten_cumulative type ", number: " print TS_eaten_num ; for testing
  ...
  if (EffectsModule) [
    if (Effects = "GUTS-SD") [ with-local-randomness[ ask turtles with [stage != "egg"] [GUTS_SD] ] ]
    if (Effects = "GUTS-IT") [ with-local-randomness[ ask turtles with [stage != "egg"] [GUTS_IT] ] ]
...
to convertBiomassesForOutput
  ...
  with-local-randomness [ ;; local randomness added here because these are only related to outputs: calling sum of turtles uses random number generator
    ...
    ifelse flag_limit_detritus  [
      ...
      if detritus_cons < 0 [ user-message "detritus consumption negative! (from 'convertBiomassesForOutput' procedure)" ]
...
to readExposureConcentrations
  ...
  ifelse file-exists? ExposureProfile [
    ...
    let entry ""
    let cnt 0
    while [ cnt < 6 ] [ ; top 6 lines of file are file info
      set entry file-read-line
;      print entry ; for testing
      set cnt cnt + 1
    ]
...
;    type "ExposureProfile column headers: " print entry ; for testing
    if (not member? PesticideName entry) [ user-message "Pesticide name does not correspond to name listed in exposure input file or file header has changed." ]
...
632
DailyPreyBiomass
IBM_TS_master_ref_CASMOC_baseline_Feb2020_TSinit.out
1
0
String
...
185
BiomassOutput
biomass_output_6Mar2020_beta0.06_50yr.txt
1
0
String
